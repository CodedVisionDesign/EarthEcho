...
export function DashboardDemo() {
  ...
  return (
    <div className="flex items-end justify-center gap-6 lg:gap-8">
      {/* ── Browser window ─────────────────────────────────── */}
      <div className="min-w-0 flex-1 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-2xl">
        {/* Chrome bar */}
        <div className="flex items-center gap-2 border-b border-gray-100 bg-gray-50 px-4 py-3">
          <div className="h-3 w-3 rounded-full bg-red-400" />
          <div className="h-3 w-3 rounded-full bg-amber-400" />
          <div className="h-3 w-3 rounded-full bg-green-400" />
          <div className="ml-3 flex-1 rounded-md bg-white px-3 py-1 text-xs text-slate transition-all duration-300">{urlPath}</div>
        </div>
                      ...
                      return (
                        <li key={item.page}>
                          <button type="button" onClick={() => navigateTo(item.page)}
                            ...
                            {active && <span className="absolute left-0 top-1/2 h-4 w-[2px] -translate-y-1/2 rounded-r-full bg-forest" />}
      ...
      <div className="hidden w-[220px] shrink-0 xl:block">
        <div className="relative mx-auto w-[220px] rounded-[2.5rem] border-[6px] border-gray-800 bg-gray-800 shadow-2xl">
          {/* Dynamic Island */}
          ...
          <div className="relative overflow-hidden rounded-[2rem] bg-white">
            ...
            <div className="flex items-center justify-between bg-white px-5 pb-1 pt-8">
              <span className="text-[8px] font-semibold text-charcoal">9:41</span>
              ...
              <div className="flex">
                {DEMO_NAV.map((item) => (
                  <button key={item.page} type="button" onClick={() => phoneNavigateTo(item.page)}
                    className={`flex flex-1 flex-col items-center gap-0.5 py-2 text-[7px] font-medium transition-colors ${phonePage === item.page ? "text-forest" : "text-slate/40"}`}>
    ...
    <div className={`relative bg-off-white ${compact ? "p-2" : "p-3 md:p-5"}`}>
      ...
      <div className={`mb-2 flex flex-wrap ${compact ? "gap-1" : "gap-1.5"}`}>
        {LOG_ACTIONS.map((action) => (
          <button key={action.category} type="button" onClick={() => onLog(action.category, action.label)}
            className={`inline-flex items-center gap-0.5 rounded-md font-medium transition-all duration-200 ${action.color} ${compact ? "px-1.5 py-0.5 text-[7px]" : "gap-1 rounded-lg px-2.5 py-1 text-[10px]"}`}>
        ...
        {(compact ? MOCK_CHALLENGES.slice(0, 2) : MOCK_CHALLENGES).map((ch) => {
          ...
          return (
            <Card key={ch.title} variant="interactive" className={`relative overflow-hidden ${compact ? "p-2" : "p-3"}`}>
              ...
              {ch.joined && !done && (
                <button type="button" onClick={() => onLog(ch.category, ch.title)}
    ...
    <div className={`bg-off-white ${compact ? "p-2" : "p-3 md:p-5"}`}>
      ...
      <Card variant="default" className="overflow-hidden p-0">
        <table className="w-full text-left">
          ...
          <tbody>
            {rows.map((e) => (
              <tr key={e.name} className={`border-b border-gray-50 transition-colors ${e.isYou ? "bg-forest/5" : "hover:bg-gray-50"}`}>
...
function Toast({ lastAction, loggedCount, compact }: { lastAction: string | null; loggedCount: number; compact: boolean }) {
  ...
  return (
    <div className={`absolute left-1/2 top-1 z-20 -translate-x-1/2 animate-fade-in rounded-lg border border-green-200 bg-green-50 shadow-md ${compact ? "px-2 py-1" : "px-3 py-1.5"}`}>
  ...
  return (
    <Card variant="interactive" className={`relative overflow-hidden ${compact ? "p-1.5" : "p-2.5 md:p-3"}`}>
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <div className={`font-bold text-charcoal transition-all duration-300 ${compact ? "text-[8px]" : "mb-0.5 text-xs md:text-sm"}`}>{humanValue}</div>
