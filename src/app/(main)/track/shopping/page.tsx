...
import { faShirt } from "@/lib/fontawesome";
...
export default async function ShoppingTrackingPage() {
  const user = await getCurrentUser();
  const category = "FASHION" as const;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        />
        <CategoryChart data={trendData} color="#2D6A4F" label={config.label} category={category} unitLabel={config.unitLabel} />
      </div>
